...
// LinuxRotationManager handles automated password rotation on Linux systems
type LinuxRotationManager struct {
...
// NewLinuxRotationManager creates a new Linux-compatible rotation manager
func NewLinuxRotationManager(backend *gmsaBackend, config *RotationConfig) RotationManagerInterface {
	ctx, cancel := context.WithCancel(context.Background())

	return &LinuxRotationManager{
		config:    config,
		status:    &RotationStatus{Status: "idle"},
		backend:   backend,
		ctx:       ctx,
		cancel:    cancel,
		logger:    log.New(log.Writer(), "[gmsa-rotation-linux] ", log.LstdFlags),
...
func (rm *LinuxRotationManager) Start() error {
	if rm.isRunning {
	...
	rm.logger.Printf("Linux-compatible automated password rotation started (check interval: %v)", rm.config.CheckInterval)
	return nil
}

// Stop stops the automated rotation process
func (rm *LinuxRotationManager) Stop() error {
	if !rm.isRunning {
	...
	rm.logger.Printf("Linux-compatible automated password rotation stopped")
	return nil
}

// rotationLoop is the main rotation loop that runs in the background
func (rm *LinuxRotationManager) rotationLoop() {
...
func (rm *LinuxRotationManager) checkAndRotate() {
...
func (rm *LinuxRotationManager) getPasswordInfoLDAP(cfg *Config) (*PasswordInfo, error) {
...
func (rm *LinuxRotationManager) parseLDAPOutput(output string) (*PasswordInfo, error) {
...
func (rm *LinuxRotationManager) parseWindowsFileTime(fileTime string) time.Time {
...
func (rm *LinuxRotationManager) needsRotation(info *PasswordInfo) bool {
...
func (rm *LinuxRotationManager) performRotation(cfg *Config) error {
	rm.status.Status = "rotating"

	rm.logger.Printf("Starting password rotation...")

	// Generate new keytab using Linux-compatible method
	newKeytabB64, err := rm.generateNewKeytabLinux(cfg)
	if err != nil {
...
// generateNewKeytabLinux generates a new keytab using Linux-compatible methods
func (rm *LinuxRotationManager) generateNewKeytabLinux(cfg *Config) (string, error) {
	...
	// Use ktutil (Linux Kerberos utility) to generate keytab
...
func (rm *LinuxRotationManager) backupCurrentKeytab(cfg *Config) error {
...
func (rm *LinuxRotationManager) testNewKeytab(cfg *Config) error {
...
func (rm *LinuxRotationManager) handleError(err error) {
...
func (rm *LinuxRotationManager) sendNotification(message string) {
...
func (rm *LinuxRotationManager) GetStatus() *RotationStatus {
...
func (rm *LinuxRotationManager) IsRunning() bool {
	return rm.isRunning
}
