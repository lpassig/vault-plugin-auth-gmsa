...
"@
    ...
    # Method 2: Use ServerCertificateValidationCallback (modern)
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                $protectedEndpoints = @(
                    "$VaultUrl/v1/sys/config", # Vault config endpoint (requires auth)
                    "$VaultUrl/v1/auth/gmsa/config", # Our gMSA config endpoint (requires auth)
                    "$VaultUrl/v1/sys/seal-status", # Vault seal status
                    "$VaultUrl/v1/sys/leader" # Vault leader endpoint
                )
                
                foreach ($endpoint in $protectedEndpoints) {
                    Write-Log "Trying SPNEGO generation with protected endpoint: $endpoint" -Level "INFO"
                ...
                # If no protected endpoint worked, try a different approach
                Write-Log "No protected endpoints triggered SPNEGO, trying alternative approach..." -Level "INFO"
                
                # Try using a fake endpoint that will definitely return 401
                $fakeEndpoint = "$VaultUrl/v1/auth/nonexistent/login"
                Write-Log "Trying SPNEGO generation with fake endpoint: $fakeEndpoint" -Level "INFO"
                
                $webRequest = [System.Net.WebRequest]::Create($fakeEndpoint)
                ...
                try {
                    $webResponse = $webRequest.GetResponse()
                    Write-Log "Fake endpoint request completed with status: $($webResponse.StatusCode)" -Level "INFO"
                    $webResponse.Close()
                } catch {
                    $webStatusCode = $_.Exception.Response.StatusCode
                    Write-Log "Fake endpoint request returned: $webStatusCode" -Level "INFO"
            ...
            # Method 2: Generate SPNEGO token using HttpClient with proper SPN targeting
            try {
                Write-Log "Generating SPNEGO token using HttpClient with proper SPN targeting..." -Level "INFO"
                ...
                try {
                    $response = $client.GetAsync($VaultUrl).Result
                    
                    if ($response -ne $null) {
                        Write-Log "HttpClient SPNEGO request completed with status: $($response.StatusCode)" -Level "INFO"
                    ...
                    if ($_.Exception.InnerException -ne $null -and $_.Exception.InnerException.Response -ne $null) {
                        $statusCode = $_.Exception.InnerException.Response.StatusCode
                        Write-Log "HttpClient SPNEGO request returned: $statusCode" -Level "INFO"
                        ...
                        if ($client.DefaultRequestHeaders.Contains("Authorization")) {
                            ...
                            if ($authHeader -and $authHeader[0] -like "Negotiate *") {
                                $spnegoToken = $authHeader[0].Substring(10) # Remove "Negotiate "
                                Write-Log "SUCCESS: Real SPNEGO token captured from HttpClient headers!" -Level "SUCCESS"
                                Write-Log "Token (first 50 chars): $($spnegoToken.Substring(0, [Math]::Min(50, $spnegoToken.Length)))..." -Level "INFO"
                                $client.Dispose()
                                return $spnegoToken
                            }
                        ...
                        Write-Log "HttpClient SPNEGO request failed: $($_.Exception.Message)" -Level "WARNING"
                    }
                }
                
                $client.Dispose()
            } catch {
                Write-Log "Method 2 (HttpClient SPNEGO generation) failed: $($_.Exception.Message)" -Level "WARNING"
            }
            ...
            # Method 4: Direct SPNEGO token generation using Windows SSPI
            # Based on research findings, we need to force SPNEGO negotiation
            Write-Log "Attempting direct SPNEGO token generation using Windows SSPI..." -Level "INFO"
            
            try {
                # Create a WebRequest that will force SPNEGO negotiation
                # The key is to make a request that will definitely return 401
                $testUrl = "$VaultUrl/v1/auth/gmsa/login"
                Write-Log "Making request to: $testUrl" -Level "INFO"
                ...
                # Make the request - this should trigger SPNEGO negotiation
                try {
                    $webResponse = $webRequest.GetResponse()
                ...
                Write-Log "Direct SPNEGO generation failed: $($_.Exception.Message)" -Level "WARNING"
            }
            
            # Method 6: Force SPNEGO negotiation by making a request that will definitely return 401
            # Based on research findings, we need to trigger Windows SSPI to generate SPNEGO tokens
            Write-Log "Attempting to force SPNEGO negotiation..." -Level "INFO"
            
            try {
                # The key insight: We need to make a request that will return 401 Unauthorized
                # This will trigger Windows SSPI to generate a SPNEGO token
                
                # Method 6A: Try a non-existent endpoint that will definitely return 404/401
                $testEndpoints = @(
                ...
                foreach ($endpoint in $testEndpoints) {
                    Write-Log "Trying to force SPNEGO negotiation with: $endpoint" -Level "INFO"
                ...
                # Method 6B: Try using a different approach - make a request to a protected endpoint
                # but without proper authentication to force SPNEGO negotiation
                Write-Log "Trying protected endpoint approach..." -Level "INFO"
                ...
                foreach ($endpoint in $protectedEndpoints) {
                    Write-Log "Trying protected endpoint: $endpoint" -Level "INFO"
