...
function Get-SPNEGOTokenCurl {
    ...
    try {
        Write-Log "Generating real SPNEGO token using curl for SPN: $TargetSPN"
        ...
        # Use curl with --negotiate to generate real SPNEGO token
        try {
            Write-Log "Using curl --negotiate to generate SPNEGO token..." -Level "INFO"
            
            # Create a temporary file to capture headers
            $headerFile = "$env:TEMP\vault-spnego-headers.txt"
            if (Test-Path $headerFile) {
                Remove-Item $headerFile -Force
            }
            
            # Use curl with --negotiate flag to trigger SPNEGO authentication
            $curlCmd = "curl --negotiate --user : -D `"$headerFile`" -s -o NUL `"$VaultUrl/v1/sys/health`""
            Write-Log "Executing: $curlCmd" -Level "INFO"
            
            $curlResult = Invoke-Expression $curlCmd 2>&1
            $curlExitCode = $LASTEXITCODE
            
            Write-Log "curl exit code: $curlExitCode" -Level "INFO"
            if ($curlResult) {
                Write-Log "curl output: $curlResult" -Level "INFO"
            }
            
            # Check if headers file was created and contains Authorization header
            if (Test-Path $headerFile) {
                $headers = Get-Content $headerFile
                Write-Log "Headers captured: $($headers.Count) lines" -Level "INFO"
                
                # Look for Authorization: Negotiate header
                $authHeader = $headers | Where-Object { $_ -like "Authorization: Negotiate *" }
                if ($authHeader) {
                    # Extract the SPNEGO token from the header
                    if ($authHeader -match "Authorization: Negotiate (.+)") {
                        $spnegoToken = $matches[1].Trim()
                        Write-Log "SUCCESS: Real SPNEGO token generated using curl!" -Level "SUCCESS"
                    ...
                    Write-Log "WARNING: No Authorization header found in curl response" -Level "WARNING"
                    Write-Log "Available headers: $($headers -join '; ')" -Level "INFO"
                }
            } else {
                Write-Log "WARNING: No headers file created by curl" -Level "WARNING"
            }
            
            return $null
            
        } catch {
            Write-Log "curl SPNEGO generation failed: $($_.Exception.Message)" -Level "WARNING"
            return $null
        }
...
function Get-SPNEGOToken {
    ...
    try {
        ...
        # Method 1: Generate real SPNEGO token using curl (Most Reliable for gMSA)
        try {
            Write-Log "Attempting SPNEGO token generation using curl..."
            
            # Check if curl is available
            try {
                $curlVersion = curl --version 2>&1 | Select-Object -First 1
                Write-Log "curl available: $curlVersion" -Level "INFO"
            } catch {
                Write-Log "WARNING: curl not available, skipping curl method" -Level "WARNING"
                throw "curl not available"
            }
            
            # Use curl with --negotiate to generate real SPNEGO token
            $spnegoToken = Get-SPNEGOTokenCurl -TargetSPN $TargetSPN -VaultUrl $VaultUrl
            if ($spnegoToken) {
                Write-Log "Real SPNEGO token generated successfully using curl"
                return $spnegoToken
            }
        } catch {
            Write-Log "curl method failed: $($_.Exception.Message)" -Level "WARNING"
        }
