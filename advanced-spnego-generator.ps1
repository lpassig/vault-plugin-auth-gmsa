...
<#
.SYNOPSIS
    Advanced Real SPNEGO Token Generator using Win32 SSPI APIs
.DESCRIPTION
    Generates real SPNEGO tokens using Win32 SSPI APIs for Vault gMSA authentication.
    Uses InitializeSecurityContext directly to establish proper security context.
.PARAMETER VaultUrl
...
# Main execution
try {
    Write-Log "=== Advanced Real SPNEGO Token Generator Started ===" -Level "INFO"
    Write-Log "Vault URL: $VaultUrl" -Level "INFO"
    Write-Log "Role: $Role" -Level "INFO"
    ...
    # Step 3: Generate real SPNEGO token using Win32 SSPI
    Write-Log "Generating real SPNEGO token using Win32 SSPI APIs..." -Level "INFO"
    $spnegoToken = New-RealSPNEGOToken -TargetSPN $SPN
